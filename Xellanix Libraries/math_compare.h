...
#define XELLANIX_MATH_COMPARE_H
...
namespace xellanix::math
{
	...
	// "equal to"
	template<typename T>
	inline constexpr xellanix::type::trait::enable_if_floating_point<T>
	...
	inline constexpr std::enable_if_t<(std::is_floating_point_v<T1> || std::is_floating_point_v<T2>) && (xellanix::type::trait::detail::is_arithmetic<T1, T2>::value), bool>
	...
	// "equal to"
	template<typename T1, typename T2>
	inline constexpr xellanix::type::trait::enable_if_integrals<T1, T2>
		equal(const T1 leftRaw, const T2 rightRaw)
	{
		using LT = typename std::conditional_t<type::trait::is_same_r_v<T1, bool>, unsigned char, T1>;
		using RT = typename std::conditional_t<type::trait::is_same_r_v<T2, bool>, unsigned char, T2>;
	...
	// "not equal to"
	template<typename T1, typename T2>
	inline constexpr xellanix::type::trait::enable_if_arithms<T1, T2>
